...
@Component({
  selector: 'app-set-calification-dialog-questions',
  templateUrl: './set-calification-dialog-questions.component.html',
  styleUrls: ['./set-calification-dialog-questions.component.css']
})
export class SetCalificationDialogQuestionsComponent {
  subjectId: Number = 0;
  examId: Number = 0;
  uploadId: Number = 0;
  ...
  constructor(
    public dialogRef: MatDialogRef<SetCalificationDialogQuestionsComponent>,
    private uploadService: UploadService,
    private router: Router,
    private _snackBar: MatSnackBar,
    private dialog: MatDialog,
    @Inject(MAT_DIALOG_DATA)
    public data: { subjectId: Number; examId: Number; uploadId: Number, questionsCalification: String[] }
  ) {
    this.subjectId = data.subjectId;
    this.examId = data.examId;
    this.uploadId = data.uploadId;
    this.questionsCalification = data.questionsCalification;
    for(let i = 0; i < this.questionsCalification.length; i++){
      this.califications.push(0);
    }
  }
